...
namespace org.SharpTiles.Expressions.Test.Math
{
    public class FloorFunctionTest
    {
        ...
        {
            Assert.That(new FloorFunction().Evaluate(6m), Is.EqualTo(6m));
        }

        [Test]
        public void Evaluate_Should_Return_6_on_input_6_dot_1()
        {
            Assert.That(new FloorFunction().Evaluate(6.1m), Is.EqualTo(6m));
        }
        ...
        {
            Assert.That(new FloorFunction().Evaluate(-6.1m), Is.EqualTo(-7m));
        }

        [Test]
        public void Evaluate_Should_Return_minus_7_on_string_input_minus_6_dot_1()
        {
            Assert.That(new FloorFunction().Evaluate((-6.1m).ToString()), Is.EqualTo(-7m));
        }
    }
}
