...
{
    public class SubStringAfterFunction : IFunctionDefinition
    {
        private static readonly FunctionArgument[] ARGUMENTS = new[]
                             {
                                 new FunctionArgument{ Type = typeof (string), Name = "source"},
                                 new FunctionArgument{ Type = typeof (string), Name = "after"}
                             };
        ...
        public object Evaluate(params object[] parameters)
        {
            ...
            string after = (string) parameters[1] ?? "";
            int index = input.IndexOf(after);
            if (index > 0)
            {
                return input.Substring(index + after.Length);
            }
            else
            {
                return "";
